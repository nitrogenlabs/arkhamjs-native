...
var FluxNative = function (_EventEmitter) {
  ...
  function FluxNative() {
    ...
    // Options
    var _this2 = _possibleConstructorReturn(this, (FluxNative.__proto__ || Object.getPrototypeOf(FluxNative)).call(this));

    options = _immutable3.default.fromJS(options);

    // Create a hash of all the stores - used for registration / de-registration
    _this2._storeClasses = (0, _immutable2.Map)();
    _this._store = _this.getSessionData('nlFlux') || (0, _immutable.Map)();
    _this2._debug = !!options.get('debug', false);
    _this2._useCache = !!options.get('cache', true);
    return _this2;
  }
  ...
  _createClass(FluxNative, [{
    ...
    value: function dispatch() {
      ...
      actions.map(function (a) {
        ...
        _this3._storeClasses.map(function (storeClass) {
          ...
          var state = _this3._store.get(name) || _immutable3.default.fromJS(storeClass.initialState()) || (0, _immutable2.Map)();
          _this3._store = _this3._store.set(name, storeClass.onAction(type, data, state) || state);

          // Save cache in session storage
          if (_this3._useCache) {
            promises.push(_this3.setSessionData('nlFlux', _this3._store));
          }
        ...
        if (_this3._debug) {
          var actionObj = _immutable3.default.fromJS(a).toJS();
    ...
    value: function getStore() {
      ...
      if (Array.isArray(name)) {
        ...
        store = this._store || (0, _immutable2.Map)();
      }
    ...
    value: function registerStore(StoreClass) {
      ...
      if (!this._storeClasses.has(name)) {
        (function () {
          ...
          // Get cached data
          if (_this4._useCache) {
            _this4.getSessionData('nlFlux').then(function (data) {
              var cache = _immutable2.Map.isMap(data) ? data : (0, _immutable2.Map)();

              // Get default values
              var state = _this4._store.get(name) || cache.get(name) || _immutable3.default.fromJS(store.initialState()) || (0, _immutable2.Map)();
              _this4._store = _this4._store.set(name, state);

              // Save cache in session storage
              _this4.setSessionData('nlFlux', _this4._store);
            });
          }
        })();
      }
    ...
    value: function setSessionData(key, value) {
      if (_immutable3.default.Iterable.isIterable(value)) {
        value = value.toJS();
      }
      ...
      return new _bluebird2.default(function (resolve, reject) {
        try {
          _reactNative.AsyncStorage.getItem(key).then(function (value) {
            resolve(_immutable3.default.fromJS(JSON.parse(value || '""')));
          });
        } catch (error) {
          reject(error);
        }
